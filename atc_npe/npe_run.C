...
TChain *tt2=new TChain("emct");
 ...
 // fout = new TFile(TString::Format("npe_time_80-160cnt_active_region0_test.root").Data(),"RECREATE");
  ...
  fout = new TFile(TString::Format("npe_time_20-60cnt_aerogel_region_test.root").Data(),"RECREATE");
  ...
  TProfile* pr=new TProfile("npesh","npesh",1000,1400778000,1465603200,0,200);        //z
  ...
  TProfile* pr3=new TProfile("npe/nperef&time","npe/nperef&time",1000,1400778000,1465603200,0,3.0);        //z
...
//           if( cnt.i>=20 && cnt.i<60 )  //цикл по числу счетчиков
           if( cnt.i>=20 && cnt.i<21 )  //цикл по числу счетчиков
//           if( (cnt.i>=80 && cnt.i<100) || (cnt.i>=140 && cnt.i<160) )  //цикл по числу счетчиков
//           if( cnt.i>=100 && cnt.i<140 )  //цикл по числу счетчиков
//           if( cnt.i>=80 && cnt.i<160 )  //цикл по числу счетчиков
//      if( cnt.i>=0 && cnt.i<80 )  //цикл по числу счетчиков
      { 
      if( t.p>1200 && t.chi2<20 && sqrt(pow(t.x0,2)+pow(t.y0,2)+pow(t.z0,2))<40 && t.nvec>4 && mu.status>=1 && mu.nhits>5 && mu.layer>=1 && tof.nhits>=1 && emc.energy>100 && emc.ncls_trk>=2 && emc.ncls>=2 && t.emc_ncls>=2 && t.emc_ncls<=4 && tof.nhits<=8 && cnt.aerogel_region!=0 && cnt.wlshit!=1 && cnt.nearwls!=1 ) 
      //if( t.p>1200 && t.chi2<20 && sqrt(pow(t.x0,2)+pow(t.y0,2)+pow(t.z0,2))<40 && t.nvec>4 && mu.status>=1 && mu.nhits>5 && mu.layer>=1 && tof.nhits>=1 && emc.energy>100 && emc.ncls_trk>=2 && emc.ncls>=2 && t.emc_ncls>=2 && t.emc_ncls<=4 && tof.nhits<=8 && cnt.active_region0!=0 && cnt.wlshit!=0 && cnt.nearwls!=0 ) 
      {         //20-40 УЮЕФЮЙЛЙ
      //if( cnt.phiin!=0&&cnt.phiout!=0 && cnt.aerogel_region0!=0 && cnt.wlshit!=1 && cnt.nearwls!=1 && cnt.npe<30 && cnt.tlen>3.5 ) {         //20-40 УЮЕФЮЙЛЙ
	      tt2->GetEntry(k);
	      if( c0.e>50 && c1.e>50 )
        {	
          if( next_run!=ev.run )
