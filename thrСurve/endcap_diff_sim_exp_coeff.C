...
ofstream of(TString::Format("change_data_Kcnt_all_31122018_%drange_endcap.dat",region).Data(),ios_base::out);   //очистка файла для последующей записи  
of.close();
...
const string DIR_CAL = TString::Format("result/data_Kcnt_all_0%drange.dat",region).Data();
//const string DIR_CAL = TString::Format("result/data_Kcnt_all_%drange.dat",region).Data();
//const string DIR_CAL = TString::Format("/home/ovtin/development/ATC/cosmic/light_collection/result/data_Kcnt_all_0%drange.dat",region).Data();
string f_in_cal;
o_str << DIR_CAL;
...
{
        cout<<cnt.at(i)<<"\t"<<Kcnt1.at(i)<<"\t"<<"\t"<<Kcnt2.at(i)<<"\t"<<"\t"<<Kcnt1.at(i)*Kcnt2.at(i)<<endl;
	ofstream myFile(TString::Format("change_data_Kcnt_all_31122018_%drange_endcap.dat",region).Data(),ios::app); 
      //if( (i>=20 && i<60) || (i>=100 && i<140) )  //цикл по числу счетчиков
      //if(Kcnt2.at(i)>0){
     // if( (i<20 || (i>=60 && i<100) || i>=140 ) )  //цикл по числу счетчиков
     // {
	if(Kcnt1.at(i)<0 || ( (i>=20 && i<60) || (i>=100 && i<140) ) )
	{
	result=Kcnt2.at(i);
	}
	else{
...
void endcap_diff_sim_exp_coeff()
{
const int n=161;
...
//const string DIR_REF = TString::Format("result_eff_cnt_single_exp_15032018_%drange.dat",region).Data();
const string DIR_REF = TString::Format("result_eff_cnt_single_exp_endcap_1range.dat").Data();
string f_in_ref;
o_str << DIR_REF;
f_in_ref = o_str.str(); o_str.str("");
