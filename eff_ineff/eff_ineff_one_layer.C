...
*/
     ...
     TString dirout="/home/ovtin/development/work_KEDR/KEDRATC/eff_ineff/output/one_layer/";
 ...
 char branchname[161];
   ...
   TProfile *pr0=new TProfile("pr0","Momentum dependence of the	amplitude for #mu",200,0,3000,0,50);
   TProfile *pr1=new TProfile("pr1","Momentum dependence of the	amplitude for K and #pi",200,0,3000,0,50);
   TProfile *pr2=new TProfile("pr2","Momentum dependence of the	for amplitude K and #pi",200,0,3000,0,50);
...
float Ksigma[kk];
   ...
   TFile *fout=0;
   TString fname;
   fname=TString::Format(dirout+"one_layer_%d_%d_endcap_eff_ineff_%d.root",first_cnt,end_cnt,sim).Data();
//   fname=TString::Format("one_layer_%d_%d_eff_ineff.root",first_cnt,end_cnt).Data();
   fout=new TFile(fname,"RECREATE");
   cout<<fname<<endl; 

   TCanvas *cc1 = new TCanvas("cc1",TString::Format(dirout+"one_layer_%d_%d_endcap_eff_ineff_%d.root",first_cnt,end_cnt,sim).Data(),1600,1200);
    //gStyle->SetOptStat(0);
   cc1->Divide(4,3);
...
{ 
   int total_entries=0;
   //if((counter>=0&&counter<20)||(counter>=60&&counter<80))
   //if((counter>=80&&counter<100)||(counter>=140&&counter<160))
...
//    for(int k=0; k<1000000; k++)                               //цикл по всем событиям в дереве
    for(int k=0; k<100000; k++)                               //цикл по всем событиям в дереве
    {
      tt1->GetEntry(k);
      tt2->GetEntry(k);
      ...
      {	     
        ...
        if( t.p<=1000000 && ATCregion!=0 && cnt.wlshit!=1 && t.nvec>4 && t.chi2<10 && sqrt(pow(t.x0,2)+pow(t.y0,2)+pow(t.z0,2))<40 && t.emc_ncls>1 && tof.nhits>=1 && mu.octant>=1 && mu.status>=1 && mu.nhits>1 && mu.layer>=1 && emc.energy>50 ){ 
//        if( t.p<=1000000 && ATCregion!=0 && cnt.wlshit!=1 && t.nvec>4 && t.chi2<10 && sqrt(pow(t.x0,2)+pow(t.y0,2)+pow(t.z0,2))<40 && t.emc_ncls>1 && tof.nhits>=1 && mu.octant>=1 && mu.status>=1 && mu.nhits>1 && mu.layer>=1 && emc.energy>50 && emc.ncls_trk>=1 ){ 
//        if( t.p<=1000000 && ATCregion!=0 && cnt.wlshit!=1 && t.nvec>4 && t.chi2<5 && sqrt(pow(t.x0,2)+pow(t.y0,2)+pow(t.z0,2))<40 && t.emc_ncls>1 && tof.nhits>=1 && mu.octant>=1 && mu.status>=1 && mu.nhits>1 && mu.layer>=1 && emc.energy>50 && emc.ncls_trk>=1 ){ //barrel
    			  tt3->GetEntry(k);
					  //if( c0.e>50 && c1.e>50 ) 
            //{ 
	      cout<<"Run="<<ev.run<<"\t"<<"Event="<<ev.event<<"\t"<<"Evdaq="<<ev.evdaq<<"\t"<<endl;
              ...
              if(t.p>1200)
              {
              	if(cnt.npe>npe_trh){num_npenotzero_cosmmu=++num_npenotzero_cosmmu;}
...
cc1->cd();
//cc1->SaveAs(TString::Format("one_layer_%d_%d_eff_ineff_canvas.root",first_cnt,end_cnt).Data());
cc1->SaveAs(TString::Format(dirout+"one_layer_%d_%d_endcap_eff_ineff_canvas_%d.root",first_cnt,end_cnt,sim).Data());
...
{
ofstream of(TString::Format("cnt%d-%d_single_npetrh%f_eff_ineff_p%d.dat",first_cnt,end_cnt,npe_trh,kk4).Data(),ios::app);         
...
*/    
   ...
   //cout<<"npe_trh="<<npe_trh<<"\t"<<"Eff="<<eff_cosmmu<<"\t"<<"Efferr="<<err_eff_cosmmu<<endl;
   cout<<"Cosmic muons with P>1.2GeV/c ==>"<<"\t"<<"npe_trh="<<npe_trh<<"\t"<<"Eff="<<eff_cosmmu<<"\t"<<"Efferr="<<err_eff_cosmmu<<endl;
   
   ofstream of2(TString::Format(dirout+"one_layer_%d_%d_npetrh%f_eff_ineff_allcnt_%d.dat",first_cnt,end_cnt,npe_trh,sim).Data(),ios::app);         
//   ofstream of2(TString::Format("one_layer_%d_%d_npetrh%f_endcap_eff_ineff_allcnt.dat",first_cnt,end_cnt,npe_trh).Data(),ios::app);         
