...
void eff_ineff_thickcnt_single_cnt()
{
...
/*
 TFile *fout=0;
 TString fname;
 fname=TString::Format("cnt_thick_%d_%d_eff_ineff.root",first_cnt,end_cnt).Data();
...
{ 
   ...
   TCanvas *cc1 = new TCanvas("cc1",TString::Format("cnt_thick_%d_%d_eff_ineff_single_cnt.root",first_cnt,end_cnt).Data(),1600,1200);
   //gStyle->SetOptStat(0);
   cc1->Divide(4,3);
...
{
	tt1->GetEntry(k);
        if( t.p<=1000000 && t.nvec>4 && t.chi2<5 && sqrt(pow(t.x0,2)+pow(t.y0,2)+pow(t.z0,2))<40 && t.emc_ncls>1 && tof.nhits>=1 && mu.octant>=1 && mu.status>=1 && mu.nhits>1 && mu.layer>=1 && emc.energy>50 && emc.ncls_trk>=1 && t.emc_ncls<=4 && tof.nhits<=8)
//  if ( t.p<=1000000 && t.nvec>4 && t.nvecxy>4 && t.nvecz>4 && t.nhits>4 && t.nhitsz>4 && t.nhitsxy>4 && t.chi2<150 && t.emc_ncls>=1 && t.emc_ncls<=3 && tof.nhits<=4 )
  {
  int j=0;
  float sum_npe=0;
    ...
    //cout<<"cnt.i="<<cnt.i<<"\t"<<"cnt.npe="<<cnt.npe<<endl; 
      ...
      //cout<<"crossing of the counter =>"<<n<<"  Nentries="<<nentr<<"\t"<<"Cnt1="<<counter<<"\t"<<"Cnt2="<<counter2<<"\t"<<"Cnt3="<<counter3<<"\t"<<endl;  
   	  if( cnt.i==counter || cnt.i==counter2 || cnt.i==counter3 )  
		  {	
       ...
       if (reg_aer==9) {ATCregion=cnt.single_active_region0;  }
    	 ...
    	 if ( ATCregion!=0 && cnt.wlshit!=1 && cnt.nearwls!=1 && j>1 )
       {
...
cc1->cd();
cc1->SaveAs(TString::Format("cnt_thick_%d_%d_%d_eff_ineff_single_cnt_canvas.root",counter,counter+1,counter+80).Data());
...
{
ofstream of(TString::Format("cnt_thick_%d_%d_npetrh%f_eff_ineff_p%d.dat",first_cnt,end_cnt,npe_trh,kk4).Data(),ios::app);         
...
ofstream of2(TString::Format("thick_eff_%d_%d_npetrh%f_eff_ineff.dat",first_cnt,end_cnt,npe_trh).Data(),ios::app);         
